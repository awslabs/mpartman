create or replace function mpartman.f_is_eligible(
    p_table_name text
)
 returns oid
 language plpgsql
        ...
        where
            ...
            (
                (
                    ...
                    vpt.part_col_data_type = ANY (mpartman.f_get_allowed_types('generalrange'))
                )
                or
                (
                    vpt.partition_strategy = 'list' and
                    vpt.part_col_data_type = ANY (mpartman.f_get_allowed_types('generallist'))
                )
            )
        ;
    end if;
