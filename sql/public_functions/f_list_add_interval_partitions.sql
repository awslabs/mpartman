create or replace function mpartman.f_list_add_interval_partitions(
    p_table_name text,
    p_interval text,
...
begin
    ...
    if (p_interval ~ '-') then
        l_sql := 'select a, b from mpartman.f_get_min_max_value_list($1, $2, ''min'') as (a text, b ' || l_type || ')';
    else
        l_sql := 'select a, b from mpartman.f_get_min_max_value_list($1, $2, ''max'') as (a text, b ' || l_type || ')';
    end if;
    ...
    l_current_start := mpartman.f_add_interval(l_start, p_interval);
