create or replace function mpartman.f_range_merge_partitions(
    p_table_name text,
    p_part_arr text[],
...
begin
	...
	-- set left boundary
        l_t := mpartman.f_get_range_bvalue(p_part_arr[l_a], 'left');
	...
	-- set right boundary
        l_t := mpartman.f_get_range_bvalue(p_part_arr[l_a], 'right');
    ...
    l_sql := 'select a from mpartman.f_get_min_max_value_range($1, null::' || l_type || ', ''max'') as (a text, b ' || l_type || ')';
    ...
    l_current_part_name := mpartman.f_set_part_name(l_only_table_name, '_p_', l_left_final_boundary);
