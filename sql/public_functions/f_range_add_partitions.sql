create or replace function mpartman.f_range_add_partitions(
    p_table_name text,
    p_bond_arr anyarray,
...
begin
    ...
    -- Check overlap
    l_t := mpartman.f_get_overlap_by_range(l_table_oid, l_type, p_bond_arr[1], p_bond_arr[array_upper(p_bond_arr, 1)]);
    if (l_t is not null) then
